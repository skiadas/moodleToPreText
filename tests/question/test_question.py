...
def readfile(filepath):
    return open(path.join(path.dirname(__file__), filepath),
                "r",
                encoding="utf-8")
...
class TestQuestionCreation(unittest.TestCase):
    ...
    def test_coderunner(self):
        ...
        self.assertEqual("<h3>Sum Number Array</h3>", question.questionText)
    ...
    def test_description(self):
        ...
        self.assertEqual("priority_queue_other_implementations_introduction",
                         question.name)
...
<p><h3>Example: Directed Graph</h3></p>""".strip()
        question: MultipleChoiceQuestion = MultipleChoiceQuestion.fromEntry(
            node)
